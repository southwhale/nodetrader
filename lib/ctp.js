...
(function() {
	...
	this.init = function(setting) {
		var clonedDefaultSetting = _.cloneDeep(this.defaultSetting);
		this.setting = _.assign(clonedDefaultSetting, setting || {});
		var brokerID = this.setting.BrokerID;
		
		// 为account添加BrokerID和InvestorID, 方便后续接口直接把account作为参数传入
		_.forEach(this.setting.accountMap, function(account) {
			account.BrokerID = brokerID;
			account.AccountID = account.UserID;
			account.InvestorID = account.UserID;

			delete account.loginCount;
		});
	...
	/**
	 * @param {Object} config 配置RegisterFront项
	 * {
	 *		md: true,
	 *		td: true
	 * }
