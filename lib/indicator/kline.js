...
var Pushing = {
    ...
    Switch : function() {
        ...
        if (Pushing.state == Pushing.State.Enable) {
            ...
            var now = 'OKEX'+'.'+chart._market_from+'.'+chart._time+'.'+chart._money_type+'.'+chart._contract_unit;
...
Chart.prototype.setCurrentCoin = function(coin) {
    this._market_from = 'f_usd_' + coin.toString();
...
Chart.prototype.updateDataAndDisplay2 = function() {
    ...
    this.setTitle();
    ChartManager.getInstance().setCurrentDataSource('frame0.k0','OKEX'+'.'+this._market_from+'.'+this._time+'.'+this._money_type+'.'+this._contract_unit);
    ChartManager.getInstance().setNormalMode();
    var f = GLOBAL_VAR.chartMgr.getDataSource("frame0.k0").getLastDate();
    if (f == -1) {
...
Chart.prototype.setCurrentContractUnit = function(contractUnit) {
    ...
    if (this._contract_unit == contractUnit) {
        return;
    }
...
ChartManager.prototype.init = function() {
    ...
    DefaultTemplate.loadTemplate("frame0.k0","OKEX","frame0.order","0.order","frame0.trade","0.trade");
...
function KLineMouseEvent() {
    $(document).ready(function(){
        ...
        $("#chart_container .chart_toolbar_tabgroup a")
            .click(function() {
                var periodName = $(this).parent().attr('name');
                switch_period(periodName);
                //点击分时埋点记录
                var eventId = "";
                switch (periodName) {
                    case '1m':
                        eventId = "click_future_k_1mins_charts";
                        break;
                    case '3m':
                        eventId = "click_future_k_3mins_charts";
                        break;
                    case '5m':
                        eventId = "click_future_k_5mins_charts";
                        break;
                    case '15m':
                        eventId = "click_future_k_15mins_charts";
                        break;
                    case '30m':
                        eventId = "click_future_k_30mins_charts";
                        break;
                    case '1h':
                        eventId = "click_future_k_1hour_charts";
                        break;
                    case '2h':
                        eventId = "click_future_k_2hour_charts";
                        break;
                    case '4h':
                        eventId = "click_future_k_4hour_charts";
                        break;
                    case '6h':
                        eventId = "click_future_k_6hour_charts";
                        break;
                    case '12h':
                        eventId = "click_future_k_12hour_charts";
                        break;
                    case '1d':
                        eventId = "click_future_k_day_charts";
                        break;
                    case '1w':
                        eventId = "click_future_k_week_charts";
                        break;
                }
                if(eventId !== "") {
                    window.parent&&window.parent.okLog.eventLog({
                        platform: 2,
                        eventId: eventId
                    });
                }
        ...
        GLOBAL_VAR.G_HTTP_REQUEST = $.ajax({
            ...
            success     : function(json) {
                ...
                if (GLOBAL_VAR.G_HTTP_REQUEST) {
                    ...
                    GLOBAL_VAR.KLineData = json.data;
...
var main = function() {
    ...
    window._set_contract_type = function(str) {
        GLOBAL_VAR.contract_type = str;
    };
    window._set_is_future_index = function(isFutureIndex) {
        GLOBAL_VAR.isFutureIndex = isFutureIndex;
        if (isFutureIndex) {
            GLOBAL_VAR.url = '/v2/futures/market/futuresIndexKlineData.do';
...
function setHttpRequestParam(mark_from, time_type, limit, since){
    var str = "symbol=" + mark_from + "&type=" + time_type;

    if (!GLOBAL_VAR.isFutureIndex) {
        str += '&contractType=' + GLOBAL_VAR.contract_type;
    }
...
function refreshTemplate() {
    GLOBAL_VAR.chartMgr = DefaultTemplate.loadTemplate(
        "frame0.k0", "OKEX"+GLOBAL_VAR.mark_from+GLOBAL_VAR.time_type,
